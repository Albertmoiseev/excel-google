...
def xlsx_opener(xlsx):
    # Open book
    book = openpyxl.load_workbook(xlsx)
    worksheet = book.active
    return worksheet

def xlsx_active_range_identifier(worksheet):
...
def xlsx_coordinates_extracter(active_cells):
...
# The date container
the_date =[]
def xlsx_data_extracter(active_cells):
    for row in active_cells:
        for cell in row:
...
def google_sheet_opener(scope='https://spreadsheets.google.com/feeds',
...
def google_coordinates_extracter(parsed_sheet):
...
def google_sheet_update(googlesheet):
    for apteka in apteki.keys():
        try:
            # Выручка
...
ws = xlsx_opener('2.xlsx')
active_cells = xlsx_active_range_identifier(ws)
xlsx_coordinates_extracter(active_cells)
xlsx_data_extracter(active_cells)
googlesheet, parsedsheet = google_sheet_opener()
google_coordinates_extracter(parsedsheet)
google_sheet_update(googlesheet)
print('Updated')

...
# Search for the active Range of cells
# Search for Pharmacy coordinates
for row in ws.iter_rows():
    for cell in row:
        # Saving the non-full Columns letters and Rows
        # range in the excel_range dict
...
for apteka in apteki_list:
    ...
    # If revenue and av cheques are found in those ranges, it would be assigned to a specific pharmacy
    apteki[apteka]['search_range'] = (first_name_cooridnate, last_name_cooridnate)



# Iterating over found range of active cells
# Searching for Revenue and Average Cheques
# Iterating
...
active_cells = ws[min_value_of_range:max_value_of_range]
for row in active_cells:
    for cell in row:
        ...
        #### Searching for Check number int
        if isinstance(cell.value, int):
            for apteka in apteki.keys():
        ...
        #### Searching for Check number float
        if isinstance(cell.value, float):
            for apteka in apteki.keys():
                if 'checks_coordinate' in apteki[apteka]:
                    if cell.coordinate[1:] == apteki[apteka]['checks_coordinate'][1:]:
...
# Find the sheet by name of month
months_words =['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август',
         'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь']
months_numbers = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']
datesdict = dict(zip(months_numbers, months_words))
name_of_the_sheet = datesdict[the_date[3:5]]
sheet = client.open("Farma2")
sheet = sheet.worksheet(name_of_the_sheet)

# Parsing
ws = sheet.get_all_values()
        ...
        if isinstance(cell, str):
            for couner, apteka in enumerate(apteki.keys()):
                ...
                if fuzz.token_sort_ratio('Красного Маяка,', cell) > 60:
                    if cell[-1] == '9':
                        print(cell)
                        apteki['Redmayak_9']['google_name_occurence_excel_format'] = (column_index + 1, row_index + 1)
                        apteki['Redmayak_9']['google_name_occurence_list_format'] = (column_index, row_index)
                        print(apteki['Redmayak_9']['google_name_occurence_excel_format'])
                    if cell[-1] == '1':
                        print(cell)
                        apteki['Redmayak_4k1']['google_name_occurence_excel_format'] = (column_index + 1, row_index + 1)
                ...
                #Для всех остальных
                    ...
                    # print('Cell identity -- Column index: ' + str(column_index+1)
                    #       + ' , Row index: ' + str(row_index + 1) +
                    #     ' Value: ' + str(cell))
                ...
                if fuzz.token_sort_ratio(the_date, cell) > 90:
                    apteki[apteka]['google_date_row_index_excel_format'] = row_index + 1
                    apteki[apteka]['google_date_row_index_list_format'] = row_index

# for k, v in apteki.items():
#         try:
#             print(k + ' -- ' + str(v['google_name_occurence_excel_format']))
#         except KeyError:
#             pass
#
# for k, v in apteki.items():
#         try:
#             print(k + ' -- ' + str(v['google_date_row_index_excel_format']))
#         except KeyError:
#             pass

#Appending info to google sheets
for apteka in apteki.keys():
    try:
        # Выручка
        sheet.update_cell(apteki[apteka]['google_date_row_index_excel_format'],
...
print(apteki['Redmayak_9']['google_name_occurence_excel_format'])
print(apteki['Redmayak_4k1']['google_name_occurence_excel_format'])
print(apteki['Lusinovskaya'])
print(apteki['Rudnevka'])